...
langage Perl6.  Il n'est pas conseillé de les suivre à la première
lecture.
...
supposons que vous êtes familier avec le langage Perl 5 car nous
...
Perl 6 est un langage réflexif. Pour la déinition de ce concept, je
vous renvoie au wikipedia.  Cela signifie notamment que tous les types
nécessaire à la compilation d'un programme son accessible du programme
lui-même.
...
parameter instead of operand. It remind us that operators are just syntactic sugar for function.
...
Le sucre syntaxique est une convention qui permet d'exprimer plus commodément quelque chose.
...
...et sucre syntaxique correspondant.
...
que pour forcer l'évaluation avant celle de l'affectation. 
...
Notons que tous les langages répandus surchargent la syntaxe d'accès à des tableaux pour
des objets dont la nature est associative. Perl 5 et Perl 6 ont une syntaxe spécifique
avec des accolades.
...
C<Positional>.  Ces rôles sont génériques paramétrés par défaut avec
la class C<Mu> mais nous ne parlont ici des rôle pour montrer que
...
paramètres formel correspondant

=head1 Quelques mot sur les rôles
