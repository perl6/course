...
Le bitvector de l'instance défini les éléments du sous-ensemble.
L'ensemble ne contient pas de bitvector puisqu'il serait redondant (composé de 1)
Le hash de S permette de nommer les éléments de l'ensemble et d'associer un bit dans le bitvector.
...
Mais on peut avoir plusieurs classes dérivés de SmartSet. Nommons les S et S'.
Supposons que nous voulons travailler sur des sous-ensembles de leur unions. Il
devront être représentés par deux bitvectors si nous voulons pouvoir croître ces ensemble.
...
le nom des ensemble. Il est même possible qu'on veuille gérer les collision sur le nom des ensembles; auquel
...
Parrot fournit la notion de PMC, PolyMorphic Container qui est grandement redondante
mais moins puissant que 6Model. Le 6model implémente un meta object protocol (MOP)
...
http://www.amazon.fr/The-Metaobject-Protocol-Gregor-Kiczales/dp/0262610744/ref=sr_1_1?ie=UTF8&qid=1335436556&sr=8-1
...
qui reprend aussi la notion de macro hygiénique inventée par ... LISP. A part la syntaxe, il vaut bien avouer que Perl 6 et
...
Mais certaines valeurs peuvent n'être accessibles que d'un d'un scope et peuvent donc
...
valeur associée, mais d'autres peuvent être crée via l'opération de binding (C<:=>) ou
...
du type B s'il est déinit dans B.
...
Donc tout l'info concernant le corps d'une intance sha1 est contenue dans ce tableau qui est
l'unique menbre de la struct <Sha1Body>, elle même enchassé dans la struct Sha1Instance.
